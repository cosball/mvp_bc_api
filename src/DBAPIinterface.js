...
exports.SkinDataFind = function(query, cb_found, cb_not_found)
{  
  ...
  .then(function (resNew) {
    ...
    if (resNew.data && resNew.data.length > 0)
      ...
      cb_not_found('Not found');
  })
  .catch(function (err) {
    // log.debug("*** SkinDataFind() " + err.message);
    cb_not_found(err.message);
  });
}
...
exports.SkinDataUpdate = function(skindata, cb_succeed, cb_failed)
{
  log.debug(`** SkinDataUpdate()`);
  // log.debug(skindata);

  const url = Config.DB_API_URL + `/skin_data/${skindata.id}?access_token=${Config.ACCESS_TOKEN}`;
  
  axios.put(url, {
    'address': skindata.address,
    'addressType': skindata.addressType,
    'status': skindata.status,
    'institutionShortName': skindata.institutionShortName,
    'reporter': skindata.reporter,
    'reason': skindata.reason,
    'remark': skindata.remark,
    'transactionHash': skindata.transactionHash,
    'createdAt': skindata.createdAt,
    'userId': skindata.userId,
    'institutionId': skindata.institutionId,
    'flag': 'P'
  })
  .then((response) => {
     // log.debug(response);
